...
$env:PYTHONPATH = 'data_pipeline;backend'
...
采集完成后可运行 `PYTHONPATH=data_pipeline;backend python -m data_pipeline.tasks.daily_update` 重建关系和评分。
...
```bash
DATABASE_URL=sqlite:///../backend/talent_graph.db  # 默认复用后端 SQLite
