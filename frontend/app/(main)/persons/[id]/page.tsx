...
import { api, LEVEL_COLOR, STRENGTH_LABEL, REL_TYPE_LABEL, parseJSON, type Person, type Experience, type Paper, type Project, type Contact, type Relationship, type Outreach, type Match } from '@/lib/api';
...
export default function PersonDetailPage() {
  ...
  useEffect(() => {
    api.get<Person>(`/api/persons/${id}`).then(setPerson).finally(() => setLoading(false));
  }, [id]);

  if (loading) return <div className="text-warm-400">加载中…</div>;
  if (!person) return <div className="text-red-500">人才不存在</div>;
      ...
      <div className="mb-6">
        ...
        <div className="surface p-6 flex items-start">
          ...
          <div className="flex gap-2">
            <button className="px-4 py-2 bg-forest-600 text-white text-sm rounded-lg hover:bg-forest-700">创建触达</button>
...
function ExperiencesTab({ personId }: { personId: string }) {
  ...
  useEffect(() => { api.get<Experience[]>(`/api/persons/${personId}/experiences`).then(setData); }, [personId]);
...
function PapersTab({ personId }: { personId: string }) {
  ...
  useEffect(() => { api.get<Paper[]>(`/api/persons/${personId}/papers`).then(setData); }, [personId]);
  return (
    <div className="space-y-3">
      {data.length === 0 ? <Empty /> : data.map((p) => (
        <div key={p.id} className="border border-warm-200 rounded-lg p-3">
          <div className="flex items-start justify-between">
            <a href={p.source_url} target="_blank" className="text-sm font-medium text-forest-700 hover:underline">{p.title}</a>
...
function ProjectsTab({ personId }: { personId: string }) {
  ...
  useEffect(() => { api.get<Project[]>(`/api/persons/${personId}/projects`).then(setData); }, [personId]);
  return (
    <div className="space-y-3">
      {data.length === 0 ? <Empty /> : data.map((p) => (
        <div key={p.id} className="border border-warm-200 rounded-lg p-3">
          <div className="flex items-center justify-between">
            <a href={p.url} target="_blank" className="text-sm font-medium text-forest-700 hover:underline">{p.name}</a>
...
function RelationshipsTab({ personId }: { personId: string }) {
  ...
  useEffect(() => { api.get<Relationship[]>(`/api/persons/${personId}/relationships`).then(setData); }, [personId]);
  return (
    <div className="space-y-2">
      ...
      {data.length === 0 ? <Empty /> : data.map((r) => (
        <Link key={r.id} href={`/persons/${r.the_other_id}`} className="flex items-center px-3 py-2.5 border border-warm-200 rounded-lg hover:bg-warm-50 transition">
...
function ContactsTab({ personId }: { personId: string }) {
  ...
  useEffect(() => { api.get<Contact[]>(`/api/persons/${personId}/contacts`).then(setData); }, [personId]);
  const typeLabel: Record<string, string> = { email: '邮箱', phone: '电话', homepage: '个人主页', github: 'GitHub', huggingface: 'HuggingFace' };
  return (
    <div className="space-y-2">
      <div className="text-xs text-warm-400 mb-2">⚠ 联系方式默认脱敏显示，查看完整内容需相应权限并记录审计日志</div>
...
function OutreachTab({ personId }: { personId: string }) {
  ...
  useEffect(() => { api.get<Outreach[]>(`/api/persons/${personId}/outreach-records`).then(setData); }, [personId]);
...
function MatchesTab({ personId }: { personId: string }) {
  ...
  useEffect(() => { api.get<Match[]>(`/api/persons/${personId}/position-matches`).then(setData); }, [personId]);
  return (
    <div className="space-y-3">
